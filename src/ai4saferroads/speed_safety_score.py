...
def assign_priority_label(
    ...
    if (
        ...
        escalate_mask = (
            (vru_exposure_score < float(config.escalation["vru_high_risk_score_threshold"]))
            & (safe_system_reference_speed_kph <= config.safe_speed_mixed_vru_kph)
            & (posted_speed_limit_kph >= float(config.escalation["posted_speed_escalation_threshold_kph"]))
        )
...
def score_segments(df: pd.DataFrame, config: ScoreConfig | None = None) -> pd.DataFrame:
    ...
    out["speed_limit_gap_score"] = compute_speed_limit_gap_score(
        posted,
        out["safe_system_reference_speed_kph"],
    )
